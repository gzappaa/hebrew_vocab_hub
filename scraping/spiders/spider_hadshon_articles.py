...
class HadshonArticlesSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        yield ArticleItem(
            title=title,
            category=category,
            audio_urls=audio_urls,  # list
            text=text,
            word_explanations=word_explanations,
        )

        self.logger.info(f"Artigo: {title[:60]}")